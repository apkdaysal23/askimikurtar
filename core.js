...
var SND = {
    ...
    playLevelMusic: function () {
        ...
        var s = this, v = 0.06;
        // Mario-style cheerful melody: C E G A G E C D E C A(low) B C
        var melody = [
...
function setupMobile(sc) {
    var w = sc.cameras.main.width, h = sc.cameras.main.height, cy = h - 60;
    sc.mL = false; sc.mR = false;
    var mb = function (x, y, t, c, sz, dn, up) {
        var b = sc.add.text(x, y, t, {
            font: 'bold ' + (sz || 26) + 'px Segoe UI', fill: '#fff',
            backgroundColor: c || '#555', padding: { x: 14, y: 9 }
        }).setScrollFactor(0).setDepth(300).setInteractive().setAlpha(0.7);
        b.on('pointerdown', function () { SND.unlock(); dn(); });
        if (up) { b.on('pointerup', up); b.on('pointerout', up); } return b;
    };
    mb(15, cy, '◀', '#333', 24, function () { sc.mL = true; }, function () { sc.mL = false; });
    mb(85, cy, '▶', '#333', 24, function () { sc.mR = true; }, function () { sc.mR = false; });
    // Sword button with custom look
    var atkBtn = sc.add.rectangle(w - 148, cy + 20, 52, 52, 0x8B0000, 0.8).setScrollFactor(0).setDepth(299).setInteractive().setStrokeStyle(2, 0xFFD700);
    sc.add.image(w - 148, cy + 20, 'sword_icon').setDisplaySize(30, 38).setScrollFactor(0).setDepth(300);
    atkBtn.on('pointerdown', function () { SND.unlock(); sc.doAttack(); });
    mb(w - 75, cy, '⬆', '#1a5276', 24, function () { sc.doJump(); });
}
...
class BootScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Slash (improved - bigger, brighter sword arc)
...
class TutorialScene extends Phaser.Scene {
    ...
    create() {
        ...
        var items = [
            ...
            ['🗡  veya  X tuşu', 'Kılıç saldırısı', '#FF5252'],
