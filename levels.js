...
// Diversified mini boss configs: [type, hp, speed, size, behavior]
// behaviors: 'patrol','charge','jump','shoot'
var MB_CFG = {
    ...
    pride: { hp: 5, spd: 60, sz: 50, beh: 'shoot', intro: '' },// no talk
...
class LevelScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Bridge platforms near gaps so player can cross by jumping
        ...
        this.mushTxt = this.add.text(10, 35, '', { font: 'bold 12px Segoe UI', fill: '#FF6B6B', stroke: '#000', strokeThickness: 2 }).setScrollFactor(0).setDepth(100);
        this.cpNotify = this.add.text(this.cameras.main.width - 10, 10, '', { font: 'bold 13px Segoe UI', fill: '#00FF88', stroke: '#000', strokeThickness: 2 }).setOrigin(1, 0).setScrollFactor(0).setDepth(100).setAlpha(0);
        ...
        this.enemies.children.iterate(function (e) {
            ...
            switch (e.beh) {
                ...
                case 'jump':
                    ...
                    break;
                case 'shoot':
                    e.setVelocityX(0);// pride doesn't move
...
class FinalBossScene extends Phaser.Scene {
    ...
    doAttack() {
        ...
        if (b && b.alive && b.dmgCD <= 0 && Phaser.Math.Distance.Between(p.x, p.y, b.x, b.y) < 100) {
            ...
            if (b.hp <= b.maxHp * 0.6 && b.phase === 1) {
                b.phase = 2; showDlg(this, 'Bu mümkün değil! Ama pes etmem!', 'Küslük-9000', 2500);
            }
            if (b.hp <= b.maxHp * 0.25 && b.phase === 2) {
                b.phase = 3; showDlg(this, 'Belki... belki hatalıydım...', 'Küslük-9000', 2500);
            }
    ...
    update(t, dt) {
        ...
        if (this.cursors.up.isDown || this.keyW.isDown || this.keySpace.isDown) this.doJump();
        if (Phaser.Input.Keyboard.JustDown(this.keyX)) this.doAttack();
        // Boss AI - ALWAYS fights, phases change behavior
        if (b && b.alive) {
            b.atkCD -= dt;
            ...
            // Jump occasionally in phase 2+
            if (b.phase >= 2 && Math.random() < 0.005 && (b.body.touching.down || b.body.blocked.down)) {
                b.setVelocityY(-400);
            }
            // Fire rockets
...
window.addEventListener('DOMContentLoaded', function () {
    new Phaser.Game({
        type: Phaser.AUTO, parent: 'gc',
        width: window.innerWidth, height: window.innerHeight,
        scale: { mode: Phaser.Scale.FIT, autoCenter: Phaser.Scale.CENTER_BOTH },
        physics: { default: 'arcade', arcade: { gravity: { y: C.GRAVITY }, debug: false } },
        input: { touch: { target: window } },
