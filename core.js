...
function setupMobile(sc) {
    ...
    // Buton boyutu daha buyuk
    var btnSize = Math.max(75, Math.min(95, h * 0.13));
    var gap = 22;
    // Sol grup %20, sag grup %80 - arasi acik, kolay erisim
    var leftGroupCenter = w * 0.20;
    var rightGroupCenter = w * 0.80;
    // Dikey konum: ekran altindan biraz yukari
    var btnY = h - btnSize * 0.6 - 12;
    ...
    function makeNavBtn(x, label, onDown, onUp) {
        ...
        bg.on('pointerdown', function (ptr) { SND.unlock(); onDown(); MOBILE_TOUCH.register(ptr.id, bg); });
    ...
    // Sol taraf: ◀ sola, ▶ saga, aralarinda gap kadar bosluk
    var lLeft  = leftGroupCenter - (btnSize + gap) * 0.5;
    var lRight = leftGroupCenter + (btnSize + gap) * 0.5;
    makeNavBtn(lLeft,  '◀', function(){ sc.mL = true;  }, function(){ sc.mL = false; });
    makeNavBtn(lRight, '▶', function(){ sc.mR = true;  }, function(){ sc.mR = false; });

    // ── Sag grup: ⚔️ ziplama ─────────────────────────────────────
    var rLeft  = rightGroupCenter - (btnSize + gap) * 0.5;
    var rRight = rightGroupCenter + (btnSize + gap) * 0.5;

    // Atilama (sol sag gruptan)
    var atkBg = sc.add.rectangle(rLeft, btnY, btnSize, btnSize, 0x7b241c, 0.75)
        .setScrollFactor(0).setDepth(300)
        .setStrokeStyle(2, 0xf1948a)
        .setInteractive({ useHandCursor: true });
    sc.add.text(rLeft, btnY, '⚔️', {
        font: Math.round(btnSize * 0.38) + 'px Arial', fill: '#ffffff'
    }).setOrigin(0.5).setScrollFactor(0).setDepth(301);
    atkBg.on('pointerdown', function () { SND.unlock(); sc.doAttack(); });

    // Ziplama (sag)
    var jumpBg = sc.add.rectangle(rRight, btnY, btnSize, btnSize, 0x1a5276, 0.75)
        .setScrollFactor(0).setDepth(300)
        .setStrokeStyle(2, 0x5dade2)
        .setInteractive({ useHandCursor: true });
    sc.add.text(rRight, btnY, '⬆', {
